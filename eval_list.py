...
def eval_list(cfgfile, weightfile, imglist):
    ...
    cpu_time = (final_time - start_time) / len(lines)
    print (start_time, final_time, len(lines),cpu_time)
    precision = 1.0 * correct / proposals
    recall = 1.0 * correct / total
    fscore = 2.0 * precision * recall / (precision + recall)
    print("%d IOU: %f, Recal: %f, Precision: %f, Fscore: %f , cpu time:%f\n" % (
    lineId - 1, avg_iou / correct, recall, precision, fscore, cpu_time ))
